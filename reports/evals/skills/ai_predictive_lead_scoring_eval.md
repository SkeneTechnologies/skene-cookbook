# Evaluation Report: ai_predictive_lead_scoring

**Session**: batch_eval
**Generated**: 2026-02-12 00:05:04

## Summary

- **Success Rate**: 0.0% (0/3)
- **Validation Pass Rate**: 0.0%
- **Auto-Act Rate**: 100.0%
- **Total Executions**: 3

## Performance Metrics

| Metric | Value |
|--------|-------|
| Average Latency | 257.45ms |
| P50 Latency | 0.98ms |
| P95 Latency | 771.15ms |
| P99 Latency | 771.15ms |
| Max Latency | 771.15ms |
| Min Latency | 0.22ms |

## Decision Breakdown

| Decision Type | Count | Percentage |
|--------------|-------|------------|
| Auto Act | 3 | 100.0% |

## Recommendations

- ⚠️ Success rate below 95% - investigate failure causes
- ⚠️ Validation pass rate below 98% - review input/output schemas
