...
class SkillAnalyzer:
    ...
    def analyze_skill_file(self, skill_json_path: Path) -> Optional[SkillAnalysis]:
        ...
        try:
            ...
            security_reqs = self._determine_security_requirements(
                risk_level, risk_factors, skill_data
            )
    ...
    def _calculate_risk_level(self, text: str, skill_data: Dict) -> Tuple[str, List[str]]:
        ...
        for tool in tools:
            ...
            if any(
                risky in tool_name for risky in ["delete", "write", "update", "execute", "admin"]
            ):
                risk_scores["High"] += 2
    ...
    def _determine_security_requirements(
        self, risk_level: str, risk_factors: List[str], skill_data: Dict
    ) -> Dict:
    ...
    def generate_metadata_yaml(self, analysis: SkillAnalysis, output_path: Path):
        ...
        metadata = {
            ...
            "categorization": {"job_function": analysis.job_function, "jtbd": analysis.jtbd},
            "composability": {"hints": analysis.composability_hints},
        }
    ...
    def generate_security_report(self, output_path: str = "reports/security_analysis.md"):
        ...
        with open(output_path, "w") as f:
            ...
            for level, count in risk_distribution.items():
                percentage = (
                    (count / len(self.analyzed_skills) * 100) if self.analyzed_skills else 0
                )
            ...
            for skill in critical_skills:
                ...
                f.write(
                    f"- **Human-in-loop:** {skill.security_requirements['human_in_loop_required']}\n\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--action",
        choices=["analyze", "report", "metadata"],
        default="analyze",
