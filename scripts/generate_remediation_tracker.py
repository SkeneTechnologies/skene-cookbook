...
class RemediationPlanner:
    ...
    def _calculate_priority(self, skill, phase, effort):
        ...
        critical_categories = ['credential_elimination', 'destructive_operations', 'payment_hardening']
    ...
    def _is_false_positive(self, risk_factors):
        ...
        low_risk = ['list', 'search', 'filter', 'sort', 'format', 'calculate', 'view', 'show', 'display']
    ...
    def _is_easy_scoping(self, risk_factors):
        ...
        return any(f in read_only for f in risk_factors) and not any(f in ['write', 'update', 'delete'] for f in risk_factors)
    ...
    def generate_tracker(self, output_path="reports/remediation_tracker.md"):
        ...
        # Sort by priority
        sorted_tasks = sorted(self.remediation_tasks, key=lambda x: x['priority_score'], reverse=True)
    ...
    def generate_csv(self, output_path="reports/remediation_tracker.csv"):
        ...
        with open(output_path, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'skill_id', 'name', 'domain', 'phase', 'priority_score',
                'current_risk', 'target_risk', 'category', 'effort_hours',
                'strategy', 'status', 'assigned_to', 'path'
            ])
            writer.writeheader()
    ...
    def generate_summary_report(self):
        ...
        print(f"\n🎯 EXPECTED IMPACT")
        print(f"   Critical Skills: {current_critical} → {target_critical} (-{reduction}, -{reduction/current_critical*100:.0f}%)")
        ...
        print(f"\n🔧 TOP CATEGORIES")
        for category, tasks in sorted(by_category.items(), key=lambda x: len(x[1]), reverse=True)[:5]:
