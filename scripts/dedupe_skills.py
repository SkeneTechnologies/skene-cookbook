...
class SkillDeduplicator:
    ...
    def generate_report(self):
        ...
        report = {
            ...
            'summary': {
                ...
                'merge_candidates': len([p for p in self.similar_pairs if p['recommendation'] == 'merge']),
                'review_candidates': len([p for p in self.similar_pairs if p['recommendation'] == 'review'])
            },
            'duplicates': {
            ...
            'unique_verified': {
                'skills': sorted(self.unique_verified, key=lambda x: x['quality_score'], reverse=True),
    ...
    def _generate_summary_markdown(self, report):
        ...
        with open(summary_path, 'w') as f:
            ...
            f.write("|----------|-------|--------|\n")
            f.write(f"| 🔴 Duplicate Groups | {report['summary']['duplicate_groups']} | DELETE duplicates |\n")
            f.write(f"| 🟡 Similar Pairs | {report['summary']['similar_pairs']} | MERGE or REVIEW |\n")
            f.write(f"| ⚠️  Incomplete Skills | {report['summary']['incomplete_skills']} | FIX metadata |\n")
            f.write(f"| ✅ Unique & Verified | {report['summary']['unique_verified_skills']} | PROMOTE to production |\n\n")
            ...
            if report['duplicates']['groups']:
                ...
                for i, group in enumerate(report['duplicates']['groups'][:10], 1):
                    ...
                    for dup in group['duplicates']:
                        f.write(f"- `{dup['skill']['skill_id']}` (similarity: {dup['similarity']:.3f})\n")
                    f.write("\n")
            ...
            top_skills = sorted(report['unique_verified']['skills'],
                              key=lambda x: x['quality_score'], reverse=True)[:20]

            f.write("| Skill ID | Domain | Quality Score |\n")
            f.write("|----------|--------|---------------|\n")
            for skill in top_skills:
                f.write(f"| `{skill['skill_id']}` | {skill['domain']} | {skill['quality_score']}/3 |\n")
    ...
    def print_summary(self):
        ...
        print(f"   Merge:     {len([p for p in self.similar_pairs if p['recommendation'] == 'merge'])} similar pairs")
        print(f"   Review:    {len([p for p in self.similar_pairs if p['recommendation'] == 'review'])} borderline cases")
