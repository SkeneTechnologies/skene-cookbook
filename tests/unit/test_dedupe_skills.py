...
def test_initialization(temp_output_directory):
    ...
    deduplicator = SkillDeduplicator(
        similarity_threshold=0.90, base_path=str(temp_output_directory)
    )
...
def test_load_skills_with_metadata(temp_skills_directory):
    ...
    skill_data = {"id": "test_with_metadata", "name": "Test Skill", "description": "Has metadata"}

    metadata_data = {"author": "Test Author", "created": "2024-01-01"}
    ...
    test_skill = next(
        (s for s in deduplicator.skills if s["skill_id"] == "test_with_metadata"), None
    )
...
def test_find_duplicates_high_similarity(temp_skills_directory):
    ...
    deduplicator = SkillDeduplicator(
        similarity_threshold=0.90, base_path=str(temp_skills_directory)
    )
...
def test_find_duplicates_no_false_positives(temp_skills_directory):
    ...
    deduplicator = SkillDeduplicator(
        similarity_threshold=0.95, base_path=str(temp_skills_directory)
    )
...
def test_different_thresholds(temp_skills_directory, threshold):
    ...
    deduplicator = SkillDeduplicator(
        similarity_threshold=threshold, base_path=str(temp_skills_directory)
    )
